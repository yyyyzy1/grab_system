...
class FoundationPose:
  ...
  def register(self, K, rgb, depth, ob_mask, ob_id=None, glctx=None, iteration=5):
    ...
    if self.debug>=2:
      ...
      ob_mask_img = Image.fromarray((ob_mask * 255.0).clip(0, 255).astype(np.uint8))
      # 保存为 PNG 文件
      ob_mask_img.save(f'{self.debug_dir}/ob_mask.png')
    ...
    if self.debug>=2:
      ...
      # 保存深度图像，先放大并转换为 uint16 类型
      depth_img = Image.fromarray((depth * 1000).astype(np.uint16))
      # 使用 PIL 保存深度图像
      depth_img.save(f'{self.debug_dir}/depth.png')
      valid = xyz_map[...,2]>=0.001
      pcd = toOpen3dCloud(xyz_map[valid], rgb[valid])
  ...
  def pick_one(self, K, rgb, depth, ob_mask, ob_id=None, glctx=None, iteration=5, extra={}):
    '''Copmute pose from given pts to self.pcd
        @pts: (N,3) np array, downsampled scene points
        '''
    set_seed(0)
  ...
  def register_test(self, K, rgb, depth, ob_mask, ob_id=None, glctx=None, iteration=5):
    '''Copmute pose from given pts to self.pcd
    @pts: (N,3) np array, downsampled scene points
    '''
    set_seed(0)
  ...
  def register_test2(self, K, rgb, depth, ob_mask, pose_data, ob_id=None, glctx=None, iteration=5):
    '''Copmute pose from given pts to self.pcd
    @pts: (N,3) np array, downsampled scene points
    '''
    set_seed(0)
