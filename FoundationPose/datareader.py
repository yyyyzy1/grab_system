...
class YcbineoatReader:
  def __init__(self,video_dir, downscale=1, shorter_side=None, zfar=np.inf):
    ...
    self.W, self.H = Image.open(self.color_files[0]).size
  ...
  def get_color(self,i):
    ...
    color_img = color_img.resize((self.W, self.H), Image.NEAREST)
  ...
  def get_mask(self, i):
    # 读取掩码图像（可能是多通道）
    mask = imageio.imread(self.color_files[i].replace('rgb', 'masks'))
    # 如果掩码是 3 通道 (RGB)，选择有内容的通道
    if len(mask.shape) == 3:
      for c in range(3):
        if mask[..., c].sum() > 0:
          mask = mask[..., c]
  ...
  def get_depth(self, i):
    # 读取深度图像，保持为单通道
    depth = imageio.imread(self.color_files[i].replace('rgb', 'depth')).astype(np.float32) / 1e3

    # 将 NumPy 数组转换为 PIL Image 对象
    depth_img = Image.fromarray(depth)

    # 使用 PIL 的 resize 方法调整大小
    depth_img = depth_img.resize((self.W, self.H), Image.NEAREST)

    # 将调整后的图像转换回 NumPy 数组进行数值操作
